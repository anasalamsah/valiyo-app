...
export function ChildPicker({
  ...
  onSelect,
  onContinue,
  stepLabel = "Pilih Anak",
  description = "Pilih profil anak yang akan menggunakan sesi ini.",
  emptyStateDescription = "Tambahkan profil anak dulu di dashboard sebelum melanjutkan.",
}: {
  ...
  if (childProfiles.length === 0) {
    return (
      <div className="rounded-[28px] bg-surface p-7 text-center shadow-sm shadow-black/5">
        ...
        <p className="mt-2 text-sm text-text-muted">{emptyStateDescription}</p>
        <Link
          href="/dashboard"
  ...
  return (
    <div className="rounded-[28px] bg-surface p-7 shadow-sm shadow-black/5">
      <p className="text-xs font-bold uppercase tracking-wider text-primary">{stepLabel}</p>
